...
/*
 2, 직원의 이름과 이메일 주소중 아이디 부분만 출력하시오 ex) 노옹철 no_hc ex) 정중하 jung_jh
*/
select emp_name 이름,
...
3, 60년대에 태어난 직원명과 년생, 보너스 값을 출력하시오 그때 보너스 값이 null인 경우에는 0 이라고 출력 되게 만드시오
 ...
 직원명    년생      보너스
ex) 선동일 1962 0.3 ex) 송은희 1963 0
*/
select emp_name 직원명,
           case
            when substr(emp_no, 8, 1) in ('1','2') then to_char(substr(emp_no, 1,2)+1900)||'년'
            when substr(emp_no, 8, 1) in ('3','4') then to_char(substr(emp_no, 1,2)+2000)||'년'
            end 년생,
           nvl(bonus, 0) 보너스
from employee;
/*
...
5, 직원명과 입사년월을 출력하시오 단, 아래와 같이 출력되도록 만들어 보시오
 ...
 직원명        입사년월
ex) 전형돈 2012년12월 ex) 전지연 1997년 3월
*/
select emp_name 직원명,
...
/*
6, 사원테이블에서 다음과 같이 조회하세요. [현재나이 = 현재년도 - 태어난년도 +1] 한국식 나이를 적용.
사원번호 사원명 주민번호 성별 현재나이
200 선동일 621235-1*** 남 57 201 송종기 631156-1*** 남 56 202 노옹철 861015-1*** 남 33
*/
select emp_id 사원번호,
          emp_name 사원명,
          emp_no 주민번호,
          case
           ...
           case
            when substr(emp_no, 8, 1)  in ('1','2') then to_char(extract(year from sysdate)-(substr(emp_no, 1,2)+1900)+1)
            when substr(emp_no, 8, 1)  in ('3','4') then to_char(extract(year from sysdate)-(substr(emp_no, 1,2)+2000)+1)
           end 현재나이
from employee;
/*
7, 직원명, 직급코드, 연봉(원) 조회 단, 연봉은 ￦57,000,000 으로 표시되게 함 연봉은 보너스포인트가 적용된 1년치 급여임
*/
select emp_name 직원명,
           job_code 직급코드,
           to_char(salary+(salary*nvl(bonus,0)), 'fmL9,999,999,999') 연봉
from employee;
...
/*
10, 직원명, 부서코드, 생년월일, 만나이 조회 단, 생년월일은 주민번호에서 추출해서, ㅇㅇㅇㅇ년 ㅇㅇ월 ㅇㅇ일로 출력되게 함.
...
select emp_name 직원명,
           ...
           end, 'yyyy/mm/dd'),'yyyy"년 "mm"월 "dd"일"') 생년월일,
           case
            when substr(emp_no, 8, 1)  in ('1','2') then to_char(extract(year from sysdate)-(substr(emp_no, 1,2)+1900))
...
*/
select count(decode(extract(year from hire_date), 1998, 1)) "1998년",
